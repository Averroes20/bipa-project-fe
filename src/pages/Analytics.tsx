...
export default function Analytics() {
  ...
  return (
    <DashboardLayout>
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-white mb-2">Speech Performance Analytics</h1>
        <p className="text-slate-400">Deep-dive multidimensional comparison of your pronunciation vs native speakers.</p>
      </div>

      {loading || !data ? (
        <div className="animate-pulse space-y-6">
           <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <div className="h-[350px] glass rounded-3xl lg:col-span-1"></div>
              <div className="h-[350px] glass rounded-3xl lg:col-span-2"></div>
           </div>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
             ...
             <div className="lg:col-span-1 glass rounded-3xl p-6 border border-slate-700/50 flex flex-col items-center">
                <h3 className="text-lg font-semibold text-white mb-2 w-full text-left">Speech Ability Radar</h3>
             ...
             <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6">
                 {["pronunciation", "fluency", "intonation", "clarity"].map((dim) => (
                    <div key={dim} className="glass rounded-3xl p-5 border border-slate-700/50">
                      <h3 className="text-base font-semibold text-white mb-2 capitalize">{dim} Comparison</h3>
                      <div className="h-[140px] w-full">
                        <ResponsiveContainer width="100%" height="100%">
                          <BarChart data={data[`${dim}Comparison`]}>
                            <CartesianGrid strokeDasharray="3 3" stroke="#334155" vertical={false} />
                            <XAxis dataKey="name" hide />
                            <YAxis domain={[0, 100]} stroke="#475569" tick={{ fill: '#94a3b8', fontSize: 11 }} width={30} />
                            <Tooltip contentStyle={{ backgroundColor: '#0f172a', borderColor: '#1e293b', fontSize: 12 }} itemStyle={{ color: '#f8fafc' }} cursor={{fill: '#1e293b'}} />
                            <Legend wrapperStyle={{ fontSize: 11, paddingTop: '5px' }} />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
             {/* AI Insights */}
             <div className="glass rounded-3xl p-6 border border-blue-500/30 flex flex-col justify-center">
                 <h3 className="text-xl font-semibold text-white mb-4 flex items-center gap-2">
                     <Brain className="text-blue-400" /> Comparison Insights
                 </h3>
                 ...
                 <div className="mt-6 space-y-4">
                     <div className="flex justify-between items-center bg-slate-800/30 rounded-lg p-3">
                         <span className="text-slate-400 text-sm">Avg Speech Rate</span>
                         <span className="text-white font-bold">{data.speakingStatistics?.wpm} WPM</span>
                     </div>
                     <div className="flex justify-between items-center bg-slate-800/30 rounded-lg p-3">
                         <span className="text-slate-400 text-sm">Avg Pause</span>
                         <span className="text-white font-bold">{data.speakingStatistics?.avg_pause_duration}s</span>
                     </div>
                 </div>
             </div>
             ...
             <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6">
                 ...
                 <div className="glass rounded-2xl p-6 border border-slate-700/50">
                    <h3 className="text-lg font-semibold text-white mb-4 flex items-center gap-2">
                        <MessageSquare size={20} className="text-emerald-400" /> Word Analytics
                    </h3>
                    {data.wordStatistics?.length > 0 ? (
                        <div className="space-y-3">
                            {data.wordStatistics.map((w: any, idx: number) => (
                                <div key={idx} className="flex justify-between items-center bg-slate-800/30 rounded-lg px-4 py-2">
                                    <span className="text-slate-200 font-medium">{w.word}</span>
                                    <div className="text-right">
                                        <div className="text-xs text-slate-400">{w.frequency} errors</div>
                                        <div className="text-sm text-red-400">{w.accuracy}% acc</div>
                                    </div>
                                </div>
                            ))}
                        </div>
                 ...
                 <div className="glass rounded-2xl p-6 border border-slate-700/50 h-full flex flex-col">
                    <h3 className="text-lg font-semibold text-white mb-4 flex items-center gap-2">
                        <Languages size={20} className="text-purple-400" /> Phonetic Deviations
                    </h3>
                    ...
                    {!phonetics || (!phonetics.mostDifficultVowels?.length && !phonetics.mostDifficultConsonants?.length) ? (
                        ...
                        <div className="space-y-5 overflow-y-auto pr-2" style={{ maxHeight: "400px" }}>
                                ...
                                <div>
                                    ...
                                    <div className="space-y-2">
                                        {phonetics.mostDifficultVowels.map((item: any, idx: number) => (
                                            <div key={idx} className="flex flex-col bg-slate-800/30 rounded-lg p-3 border border-red-500/20 gap-2">
                                                <div className="flex justify-between items-center">
                                                    <span className="bg-red-500/20 border border-red-500/30 text-red-400 text-sm font-bold px-2 py-0.5 rounded flex items-center gap-1">🟥 /{item.phoneme}/</span>
                                                    <span className="text-slate-200 text-sm font-medium">Accuracy: {item.accuracy}%</span>
                                                </div>
                                                <div className="grid grid-cols-3 gap-2 text-xs">
                                                    <div>
                                                        <p className="text-slate-500">Mistakes</p>
                                                        <p className="text-red-400 font-medium">{item.mistakes}</p>
                                                    </div>
                                                    <div>
                                                        <p className="text-slate-500">Occurrences</p>
                                                        <p className="text-slate-300 font-medium">{item.occurrences}</p>
                                                    </div>
                                                    <div>
                                                        <p className="text-slate-500">Improvement</p>
                                                        <p className="text-blue-400 font-medium">{item.improvement > 0 ? '+' : ''}{item.improvement}%</p>
                                                    </div>
                                                </div>
                                            </div>
                                        ))}
                            ...
                            {phonetics.mostImprovedSounds?.length > 0 && (
                                <div>
                                    <h4 className="text-xs font-medium text-slate-400 mb-2 uppercase tracking-wider">Most Improved Sounds</h4>
                                    <div className="flex flex-wrap gap-2">
                                        {phonetics.mostImprovedSounds.map((item: any, idx: number) => (
                                            <div key={idx} className="flex items-center gap-1.5 bg-blue-500/10 rounded-full px-3 py-1 border border-blue-500/20">
                                                <span className="text-blue-400 text-xs font-bold">🟦 /{item.phoneme}/</span>
                                                <span className="text-slate-300 text-xs">+{item.improvement}%</span>
                                            </div>
                                        ))}
                                    </div>
                                </div>
